...
namespace analog
{
    ...
    stmcpp::adc::adc<stmcpp::adc::peripheral::adc3> adc3 (stmcpp::adc::resolution::sixteenBit, stmcpp::adc::dataManegment::oneShotDMA, true, false, 16, 0, 4, true, false, true);
    // Set up the channels to be measured
    ...
    static constexpr std::array adc3_sequence = {adc3_fastic1_vmon, adc3_vbat, adc3_vsense, adc3_vrefint};
    // Create a buffer to store the measurements
    ...
    // Index used to indicate which ADC is sampling 
    // Only one ADC is working at a time to avoid VREF spikes
    bool adcIndex = false;

    void init(){
        // Set up timer 15 used to trigger the ADCs to generate an event at 100Hz
        stmcpp::reg::write(std::ref(TIM15->PSC), 240-1);   
        ...
        // Start ADC3 - we first need to measure the internal reference to calibrate the VREFBUF
        adc3.startRegular();
    ...
    void calibrateVoltageMultiplier(){
        voltageMultiplier = (((double)vrefIntCalibration * (3.3 / 65535)) / (double)adc3_measurements[3]);
    }
...
extern "C" void TIM15_IRQHandler() {
    ...
    For some reason unknown to me, the ADC needs to be configured in the DMA single shot mode
    in order to always fill the buffer indexes in the same order as the ADC sequence. 
    When the ADC is configured in the circular mode, the buffer indexes are shifted by one position after each DMA transfer.
    This behavior makes it impossible to trigger the ADC directly by the hardware trigger
    thus it needs to be triggered by the timer update interrupt.

    */

    if (stmcpp::reg::read(std::ref(TIM15->SR), TIM_SR_UIF)) {
        // Clear the update interrupt flag
        stmcpp::reg::clear(std::ref(TIM15->SR), TIM_SR_UIF);

        // Start ADC conversions
        if(analog::adcIndex){
            analog::adc3.startRegular();
            analog::adcIndex = false;
        } else {
            analog::adc2.startRegular();
            analog::adcIndex = true;
        }
    }
